...
var Blackbox = function (options) {
  ...
  if (PathTools.isMac())
    // Ignore "/home" on macOS (but not on Unix/Linux).
    this.ignorePaths["/"].home = 1;

  // Media file detection regex (pre-compiled since MuJS regexps are slow).
  this.mediaRgx =
    /\.(?:mp[234]|m4[av]|mpe?g|m[12]v|web[mp]|mk(?:[av]|3d)|h?264|qt|mov|avi|xvid|divx|wm[av]|asf|pcm|flac|aiff|wav|aac|dts|e?ac3|dat|bin|vob|vcd|mt[sv]|m2ts?|ts|flv|f4[vp]|rm(?:vb)?|3(?:gp|iv)|h?dv|og[gmv]|jpe?g|png|bmp|gif)(?:\.part)?$/i;

  // Include the user's custom inclusion regex (if available and non-empty).
  var includeRgx = options.includeRegex
    ? Utils.trim(options.includeRegex)
    : null;
  if (includeRgx) {
    try {
      ...
      throw (
        "User's include-regex: \"" +
        e.message.replace("regular expression: ", "") +
        '"'
      );
    }
  }
  ...
  this.showHelpHint =
    typeof options.showHelpHint === "boolean" ? options.showHelpHint : true;
  this.menu = new SelectionMenu({
    // Throws if bindings are illegal.
  ...
  if (favOpt && favOpt.length) {
    try {
      ...
      // Treat as non-fatal.
      this._showError(
        "Blackbox: Invalid favorites option value: " + e + ".",
        3
      );
    }
  }
...
Blackbox.prototype.flipPage = function (forcePage) {
  if (
    ...
    (forcePage === "none" && this.currentPage === null)
  )
...
Blackbox.prototype.addFavorite = function (path) {
  ...
  var pathSep = PathTools.pathSep(),
    pl = path.length;
  if (!pl) throw "Empty path";
  if (pl > 1 && path.charAt(pl - 1) === pathSep)
    // Allows "/" (root).
...
Blackbox.prototype._showError = function (err, durationSec) {
  ...
  else mp.osd_message(err, durationSec);
};
...
Blackbox.prototype._registerCallbacks = function () {
  ...
  var processSelection = function () {
    ...
    if (!selection.targetPath) return null; // Abort since there is no selection.
    ...
    switch (selection.menuPage) {
      ...
      default:
        mp.msg.error(
          "processSelection: Unknown menu page: " + selection.menuPage
        );
    ...
    if (!targetType) {
      // No other type assigned. So it's a disk-file.
      ...
      if (targetType === "missing") {
        ...
        browser._showError(
          ...
          "Blackbox: " +
            (selection.itemType === "file" ? "File" : "Target") +
  ...
  var playlistRemove = function (itemPos, showRemoval) {
    ...
    if (playlistCount >= 2) {
      ...
      if (!removeItem) return; // Safeguard against failure to fetch that offset.
      ...
      if (removeItem.current && removePos === playlistCount - 1) {
        browser.menu.showMessage(
          "Cannot remove the final playlist item, because it is playing."
        );
        return;
      }
      ...
      var c = browser.menu.useTextColors,
        removedTitle = removeItem.title
          ? removeItem.title
          : browser._shrinkFilename(removeItem.filename),
        clearSelectionPrefix = false;
      if (
        browser.currentPage !== "playlist" &&
        !PathTools.isWebURL(removeItem.filename)
      ) {
      ...
      // Shows if 1 left, does nothing if 0.
      // Don't remove the only remaining playlist item (would quit mpv).
      browser.menu.showMessage(
        "Cannot remove the only remaining playlist item."
      );
    }
  };
  ...
  browser.menu.setCallbackMenuLeft(function () {
    ...
    if (newNavTime - browser.lastNavTime < 200)
      // 0.2s
      return;
    ...
    switch (browser.currentPage) {
      case "files":
        ...
        else browser.menu.renderMenu();
        break;
      case "playlist":
        var selection = processSelection();
        if (
          selection &&
          selection.targetType === "playlist" &&
          selection.targetPath >= 1
        )
  ...
  browser.menu.setCallbackMenuRight(function () {
    ...
    switch (selection.targetType) {
      ...
      case "file":
        ...
        if (
          lastItem &&
          !PathTools.isWebURL(lastItem.filename) &&
        ...
        break;
      case "playlist":
        if (selection.targetPath >= 1)
          // Ensure target isn't a special ID.
...
Blackbox.prototype._shrinkFilename = function (path) {
  if (!PathTools.isWebURL(path))
    // Shrink local filenames.
...
Blackbox.prototype._generateLegalPath = function (path) {
  ...
  if (typeof path !== "string")
    // Re-use current if no new path provided.
  ...
  if (typeof path !== "string" || !path.length) {
    ...
    if (playingFile.pathName) {
      ...
      if (playingFile.baseName) overrideSelect = playingFile.baseName;
    }
  }
  if (typeof path !== "string" || !path.length) {
    if (overrideSelect === false)
      // Only toggle if no name-override exists.
      overrideSelect = true;
...
Blackbox.prototype.getSelection = function () {
  ...
  if (selection.menuPage === null) return selection;
  ...
  switch (selection.menuPage) {
    case "files":
      ...
      if (selectedItem.length)
        selection.targetPath = PathTools.getSubPath(
          this.currentPath,
          selectedItem
        );
      break;
    case "favorites":
      ...
      selection.targetPath =
        selection.item !== this.currentlyPlayingStr
          ? PathTools.makePathAbsolute(selection.item)
          : selection.item;
      break;
    case "playlist":
      var itemTitle = "",
      ...
      if (match) {
        ...
        if (playlist && playlist.length) {
          ...
          for (var i = 0; i < testOffsets.length; ++i) {
            ...
            if (testIdx < 0 || testIdx >= playlist.length) continue;
            ...
            var thisName = playlist[testIdx].title
              ? playlist[testIdx].title
...
Blackbox.prototype.navigateFav = function (selectEntry) {
  ...
  if (this.menu.getMetadata().type !== "favorites") {
    ...
    if (selectEntry !== this.currentlyPlayingStr) ++initialSelectionIdx; // Start at 2nd item (after "current").

    for (i = 0; i < this.favoritePaths.length; ++i) {
      fav = this.favoritePaths[i];
      menuOptions.push(fav);
      if (selectEntry === fav) initialSelectionIdx = menuOptions.length - 1;
    }

    this.menu.getMetadata().type = "favorites";
    this.menu.setTitle(
      (menuOptions.length === 0 ? "[empty] " : "") + "Blackbox Favorites"
    );
...
Blackbox.prototype.navigatePlaylist = function (playlist, keepPosition) {
  ...
  if (playlist)
    for (i = 0; i < playlist.length; ++i) {
      entryPath = playlist[i].title
        ? playlist[i].title
        : this._shrinkFilename(playlist[i].filename);
      entryIsPlaying = playlist[i].current;
      entryIndex = i + 1;
      if (entryIsPlaying) activeIndex = entryIndex;
      entryText =
        (entryIsPlaying ? "=" + entryIndex + "=" : entryIndex) +
        ": " +
        entryPath;
      menuOptions.push(entryText);
      if (entryIsPlaying && !keepPosition)
        // Found the playing item?
  ...
  this.menu.setTitle(
    "[" +
      (menuOptions.length <= extraMenuOptions
        ? "empty"
...
Blackbox.prototype.navigateDir = function (path, selectEntry, forceRefresh) {
  ...
  try {
    ...
    if (legalPathInfo.overrideSelect !== false)
      selectEntry =
        typeof legalPathInfo.overrideSelect === "string"
          ? legalPathInfo.overrideSelect
          : null;
    ...
    if (
      ...
      var i,
        dir,
        file,
        dirContents = new PathIndex(path, {
          // Throws if bad path.
          skipDotfiles: true,
      ...
      for (i = 0; i < dirContents.dirs.length; ++i) {
        ...
        if (ignorePaths && ignorePaths[dir]) continue; // Hide (skip) this directory.
        dir += "/"; // Append slash to signify that it is a directory.
        menuOptions.push(dir);
        if (selectEntry === dir) initialSelectionIdx = menuOptions.length - 1;
      }
      for (i = 0; i < dirContents.files.length; ++i) {
        file = dirContents.files[i];
        menuOptions.push(file);
        if (selectEntry === file) initialSelectionIdx = menuOptions.length - 1;
      }

      var helpPrefix = "";
      if (this.showHelpHint && this.currentPath === null) {
        // 1st browse.
        var helpKeys = this.menu.keyBindings["Menu-Help"].keys;
        if (helpKeys.length) helpPrefix = "[" + helpKeys[0] + " for help] ";
      }
      ...
      this.menu.getMetadata().type = "files";
      this.menu.setTitle(
        helpPrefix + (menuOptions.length === 0 ? "[empty] " : "") + path
      );
      this.menu.setOptions(menuOptions, initialSelectionIdx);
    }
    ...
    this._showError(
      'Blackbox: Unable to access directory "' + path + '".',
      0.8
    );
  }
};
...
Blackbox.prototype.switchMenu = function (forcePage) {
  if (typeof forcePage === "string") {
    ...
    if (forcePage === this.currentPage) forcePage = "none";
  ...
  if (this.currentPage === null) this.menu.hideMenu();
  else {
